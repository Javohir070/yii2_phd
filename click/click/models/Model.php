...
class Model{
    ...
    public function find_by_id($payment_id){
        // make sql query
        $query = 'SELECT * FROM `orders` WHERE id = ' . $payment_id;
    ...
    public function find_by_token($token){
        // make sql query
        $query = 'SELECT * FROM `orders` WHERE token = "' . $payment_id . '"';
        // prepare the query to execute
        $statement = $this->conn->prepare($query);
        // execute the statement
        $statement->execute();
    ...
    public function find_by_merchant_trans_id($merchant_trans_id){
        // make sql query
        $query = 'SELECT * FROM `orders` WHERE id = ' . $merchant_trans_id . ' and `type`=\'click\'';
        // prepare the query to execute
        $statement = $this->conn->prepare($query);
        // execute the statement
        $statement->execute();
        ...
        fwrite($fp,json_encode($res).PHP_EOL);
    ...
    public function update_by_id($payment_id, $arguments = []){
        ...
        $sets = sets($arguments);
        // make query
        $query = "Update `orders` SET $sets WHERE id = $payment_id";
        $fp = fopen('a.log', 'a');
    ...
    public function update_by_token($token, $arguments){
        ...
        $sets = sets($arguments);
        // make query
        $query = "Update `orders` SET $sets WHERE token = '$token'";
        // prepare query to execute
        $statement = $this->conn->prepare($query);
        // execute the statement
        $statement->execute();
